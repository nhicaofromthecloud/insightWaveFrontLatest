...
export function OverviewContent() {
  ...
  if (reviewData.isLoading || customerData.isLoading) {
    return <div>Loading...</div>;
  }

  const averageScore =
    reviewData.data.reviews.reduce((acc, review) => {
      return acc + review.score;
    }, 0) / reviewData.data.reviews.length;
    ...
    <>
      <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{customerData.data.length}</div>
            <p className="text-xs text-muted-foreground">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-7">
        <div className="col-span-4">
          <AreaGraph reviews={reviewData.data.reviews} />
        </div>
        <Card className="col-span-4 max-h-[50vh] w-full overflow-scroll md:col-span-3 md:max-h-[69.5vh]">
          <CardHeader className="sticky top-0 border-b bg-card text-card-foreground">
            <CardTitle>Recent reviews</CardTitle>
            <CardDescription>
              {`You have ${reviewData.data.reviews.length} reviews this month.`}
            </CardDescription>
          </CardHeader>
          <CardContent className="overflow-scroll bg-card pt-6">
            <RecentReviews
